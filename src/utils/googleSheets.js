...
/**
 * Fetches all players with their scores and answers (columns A, B, and C)
 * @param {string} sheetId - The Google Sheet ID from the URL
 * @param {string} apiKey - Your Google API key
 * @returns {Promise<Array>} Array of objects with name, score, answer, and row
 */
export const fetchAllPlayersWithScores = async (sheetId, apiKey) => {
  try {
    const range = 'Players!A2:C11';
    const url = `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${range}?key=${apiKey}`;
    const response = await axios.get(url);
 ...
 * @param {string} sheetId - The Google Sheet ID from the URL
 * @param {string} apiKey - Your Google API key
 * @returns {Promise<{category: string, score: number, categoryCell: string, scoreCell: string}>} Current category, score, and cell references
 */
export const fetchCurrentCategoryAndScore = async (sheetId, apiKey) => {
  try {
    const range = 'Points!H2:H4';
    const url = `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${range}?key=${apiKey}`;
    const response = await axios.get(url);
    ...
    return { category: '', score: 0, categoryCell: 'A1', scoreCell: 'A2' };
  } catch (error) {
...
/**
 * Resets the entire game - clears all scores and answers, resets cell references
 * @returns {Promise<void>}
 */
export const resetGame = async () => {
  try {
    ...
    console.log('Sending game reset request...');
    console.log('Request body:', JSON.stringify({ action: 'resetGame' }));
    ...
    console.log('Game reset request sent successfully');
